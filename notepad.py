...
class Notebook:
    def __init__(self):
        ...
        self.file.add_command(label = "Nuevo archivo", accelerator = "Ctrl+Q", command = self.new_file)
    ...
    def new_file(self):
        if self.text.get("1.0", "end - 1 chars") != self.content:
            if self.filename == None:
                self.result = messagebox.askyesnocancel(title = "Cambios sin guardar", message = f"Antes de continuar, ¿desea guardar el archivo?")
            else:
                self.result = messagebox.askyesnocancel(title = "Cambios sin guardar", message = f"Antes de continuar, ¿desea guardar los cambios hechos a {self.filename}?")
            if self.result != None:
                if self.result:
                    self.save_file()
                self.utf_8_file = True
                self.check_file_coding()
                self.filename = None
                self.root.title("Sin título")
            ...
            if not self.file_error:
                if self.text.get("1.0", "end - 1 chars") != self.content:
                    ...
                    if self.result != None:
                        if self.result:
                            self.save_file()
                        self.utf_8_file = self.utf_8_file_
                        self.check_file_coding()
